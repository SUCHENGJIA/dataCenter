...
void crtsql()
{
    ...
    deleterchr(strinsertp1, ',');
    deleterchr(strinsertp2, ','); // 把最后一个多余的逗号删除。
    ...
    for (auto &aa : tcols.m_vallcols) // 遍历表全部字段的容器。
    {
        ...
        if (strcmp(aa.colname, "upttime") == 0)
        {
            strupdatesql = strupdatesql + "upttime=sysdate,";
            continue;
        }
